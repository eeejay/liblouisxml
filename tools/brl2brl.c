...
/* translate braille files into text or xml using liblouisxml*/
  ...
  strcat (tempFileName, "brl2brl.temp");
  ...
  if (charsRead > 2)
    {
      ...
      strcpy (temp2FileName, ud->user_path);
      strcat (temp2FileName, "brl2brl2.temp");
      if ((lbx_backTranslateFile (configFileName, tempFileName, 
temp2FileName)) != 1)
exit (1);
      if (ud->back_text == html)
	lbx_translateFile (configFileName, temp2FileName, outputFileName);
      else
	lbx_translateTextFile (configFileName, temp2FileName, outputFileName);
    }
  lbx_free ();
  return 0;
}
