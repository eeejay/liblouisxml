...
/* translate xml and text files into braible using liblouisxml*/
  ...
  if (argc > curarg)
	...
	switch (argv[curarg][1] | 32)
	  ...
	  case 'h':
	    printf
("Usage: any2brl [-l] [-f configFile] [inputFile] [outputFile]\n");
	    ...
	    printf ("any2brl with no argumenst takes input on stdin\n");
	    printf ("and gives output on stdout.\n");
	    exit (0);
	  case 'l':
	    logFileName = "any2brl.log";
	    break;
	  case 'f':
	    if (argc < (curarg + 1))
	      {
  ...
  strcat (tempFileName, "any2brl.temp");
