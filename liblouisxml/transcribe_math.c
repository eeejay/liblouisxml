...
static int mathEmptyElement (xmlNode * node, int action);
static void mathTrans (void);
static void mathText (xmlNode * node, int action);
static void mathCdata (xmlNode * node);
...
{
  int k;
  for (k = 0; k < ud->text_length && ud->text_buffer[k] <= ' '; k++);
  if ((k + 1) == ud->text_length)
    {
      ud->text_buffer[k + 1] = ud->text_buffer[k];
      ud->text_buffer[k] = 0x009a;	/*Single character */
      ud->text_length++;
    }
...
static int
mathEmptyElement (xmlNode * node, int action)
{
  push_sem_stack (node);
  switch (ud->stack[ud->top])
    {
